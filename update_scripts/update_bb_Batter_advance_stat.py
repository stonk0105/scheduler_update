"""
update_bb_Batter_advance_stat.py
基於 bb_BallsStat_Batted Ball_P.ipynb 重構的投手擊球統計更新腳本
更新 bb_Batter_advance_stat 資料表
"""
...
# 設定日誌
log_file_path = os.path.join(os.path.dirname(__file__), 'update_bb_Batter_advance_stat.log')
logging.basicConfig(
    level=logging.INFO,
...
def update_specific_table():
    """
    更新 bb_Batter_advance_stat 資料表的主要函數
    """
    logger.info("開始更新 test_bb_Batter_advance_stat 測試資料表...")
    ...
    try:
        ...
        bb_Batter_advance_stat = pd.DataFrame(rows)
        ...
        try:
            ...
            logger.info("檢查 test_bb_Batter_advance_stat 測試表...")
            cursor.execute("SHOW TABLES LIKE 'test_bb_Batter_advance_stat'")
            table_exists = cursor.fetchone()
            
            if table_exists:
                logger.info("測試表已存在，清空原有資料...")
                cursor.execute("DELETE FROM test_bb_Batter_advance_stat")
                db.commit()
            ...
            with write_engine.connect() as con:
                bb_Batter_advance_stat.to_sql(
                    "test_bb_Batter_advance_stat", 
                    con, 
                    index=False, 
                    if_exists="append", 
                    chunksize=10000
            ...
            logger.info(f"成功更新 test_bb_Batter_advance_stat 測試資料表，共 {len(bb_Batter_advance_stat)} 筆記錄")
            
            # 輸出 CSV 檔案
            csv_filename = f"test_bb_Batter_advance_stat_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            csv_path = os.path.join(os.path.dirname(__file__), csv_filename)
            bb_Batter_advance_stat.to_csv(csv_path, index=False, encoding='utf-8-sig')
        ...
        logger.error(f"更新 test_bb_Batter_advance_stat 測試資料表時發生錯誤: {e}")
