"""
update_bb_BallsStat_BattedBall_P.py
基於 bb_BallsStat_Batted Ball_P.ipynb 重構的投手擊球統計更新腳本
更新 bb_BallsStat_BattedBall_P 資料表
"""
...
# 設定日誌
log_file_path = os.path.join(os.path.dirname(__file__), 'update_bb_BallsStat_BattedBall_P.log')
logging.basicConfig(
    level=logging.INFO,
...
def calculate_pitcher_yearly_stats(stock):
    """
    計算打者各年度和球種的擊球統計數據
    """
...
def calculate_pitcher_career_stats(stock):
    """
    計算投手生涯投球統計數據
    """
    player = list(set(stock['Pitcher']))
    ...
    for i in tqdm(range(0, len(player))):
        df = stock[(stock['Pitcher'] == player[i])].reset_index(drop=True)
        try:
            # print(len(df))
            new_row = {
                '_id': ' ',
...
def update_specific_table():
    """
    更新 bb_BallsStat_BattedBall_P 資料表的主要函數
    """
    ...
    try:
        ...
        # 計算投手年度擊球統計
        df_yearly = calculate_pitcher_yearly_stats(stock)
        
        # 計算投手生涯擊球統計
        df_career = calculate_pitcher_career_stats(stock)
        
        # 合併年度和生涯統計
        bb_BallsStat_BattedBall_P = pd.concat([df_yearly, df_career], ignore_index=True)
        
        if len(bb_BallsStat_BattedBall_P) == 0:
            logger.warning("沒有產生任何統計數據")
            return
        
        logger.info(f"產生 {len(bb_BallsStat_BattedBall_P)} 筆投手擊球統計記錄")
        ...
        try:
            ...
            logger.info("檢查 test_BallsStat_BattedBall_P 測試表...")
            cursor.execute("SHOW TABLES LIKE 'test_BallsStat_BattedBall_P'")
            table_exists = cursor.fetchone()
            
            if table_exists:
                logger.info("測試表已存在，清空原有資料...")
                cursor.execute("DELETE FROM test_BallsStat_BattedBall_P")
                db.commit()
            ...
            with engine.connect() as con:
                bb_BallsStat_BattedBall_P.to_sql(
                    "test_BallsStat_BattedBall_P", 
                    con, 
                    index=False, 
                    if_exists="append", 
                    chunksize=10000
            ...
            logger.info(f"成功更新 test_BallsStat_BattedBall_P 測試資料表，共 {len(bb_BallsStat_BattedBall_P)} 筆記錄")
            
            # 輸出 CSV 檔案
            csv_filename = f"test_BallsStat_BattedBall_P_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            csv_path = os.path.join(os.path.dirname(__file__), csv_filename)
            bb_BallsStat_BattedBall_P.to_csv(csv_path, index=False, encoding='utf-8-sig')
        ...
        logger.error(f"更新 test_BallsStat_BattedBall_P 測試資料表時發生錯誤: {e}")
